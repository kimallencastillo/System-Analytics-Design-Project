...
<?php
    if (isset($_POST['submit-search'])) {
        ...
        $sql = "SELECT * FROM applicantdb WHERE usrnmapp LIKE '%$search%' OR appID LIKE '%$search%'";
        ...
        if ($queryResult > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                echo "<a href='books.php?usrnmapp=".$row['usrnmapp']."&appID=".$row['appID']."'><div class='book-box'>
                    <h3 style='color:#eaeae8'>Applicant ID: ".$row['appID']."</h3>
                    <p style='color:#eaeae8'>Username: ".$row['usrnmapp']."</p>
                </div></a>";
            }
        } else {
